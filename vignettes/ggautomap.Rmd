...
```{r text-inset, fig.height = 3, fig.width = 7, eval=F}
covid_cases_nsw %>%
  ggplot(aes(location = lhd)) +
  geom_choropleth() +
  geom_boundaries(feature_type = "nswgeo.lhd") +
  geom_inset_frame() +
  geom_sf_label_inset(aes(label = lhd), stat = "automap_coords", data = ~ slice_head(.x, by = lhd)) +
  coord_automap(datum = NA, inset = configure_inset(
    centre = "Western Sydney", radius = 60, units = "km",
    scale = 3.5, translation = c(350, 0))) +
  labs(x = NULL, y = NULL) +
...
```{r text-repel, fig.height = 3, fig.width = 7, eval=F}
library(ggrepel)
